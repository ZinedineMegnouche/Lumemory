...
extension HomeDetailViewModel: HMAccessoryBrowserDelegate {
    ...
    func addAccesoryToHome(accessory: HMAccessory) {
        homeKitStorage.homes.first { $0.uniqueIdentifier == home.id }?
            .addAccessory(accessory, completionHandler: { err in
            if let err {
                self.textAlert = "impossible d'ajouter l'accessoire \(err)"
            } else {
                let isLightBulbHue = accessory.services.contains {
                    $0.serviceType == HMServiceTypeLightbulb && $0.characteristics.contains {
                    $0.characteristicType == HMCharacteristicTypeHue
                }
                }
